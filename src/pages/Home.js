...
import { getProducts, getBanners } from '../store';
...
function AssuredBadge() {
  return (
    <div style={{ display:'inline-flex', alignItems:'center', gap:3, background:'#2874f0', borderRadius:3, padding:'2px 6px' }}>
...
export default function Home() {
  ...
  useEffect(() => {
    const id = setInterval(() => setSlide(s => (s + 1) % banners.length), 3500);
    return () => clearInterval(id);
  }, [banners.length]);
  ...
  return (
    <div style={{ fontFamily:'Roboto,Helvetica,Arial,sans-serif', background:'#f1f2f4', maxWidth:480, margin:'0 auto', minHeight:'100vh' }}>
        ...
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:10 }}>
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              ...
              <div>
                <div style={{ color:'#fff', fontWeight:900, fontSize:20, fontStyle:'italic', lineHeight:1.1 }}>Flipkart</div>
          ...
          <svg width="22" height="22" viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg" style={{ marginTop:4 }}>
      ...
      {/* ── Category strip ── */}
      <div style={{ background:'#fff', padding:'12px 4px', display:'flex', justifyContent:'space-around', borderBottom:'1px solid #f0f0f0' }}>
        {CATEGORIES.map(c => (
          <div key={c.label} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:5, cursor:'pointer', minWidth:52 }}>
            <div style={{ width:54, height:54, borderRadius:'50%', background:c.bg, display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
              <img src={c.img} alt={c.label} style={{ width:42, height:42, objectFit:'contain' }}
      ...
      {/* ── Carousel ── */}
      <div style={{ position:'relative', overflow:'hidden', background:'#fff', userSelect:'none' }}>
        <img src={banners[slide]} alt="banner"
          style={{ width:'100%', maxHeight:200, objectFit:'cover', display:'block' }}
          onError={e => { e.target.style.minHeight='140px'; e.target.style.background='#2874f0'; e.target.alt=''; }} />
        <button onClick={() => setSlide(s => (s - 1 + banners.length) % banners.length)}
          style={{ position:'absolute', left:0, top:'50%', transform:'translateY(-50%)', background:'rgba(255,255,255,.82)', border:'none', width:28, height:52, cursor:'pointer', fontSize:22, color:'#555', display:'flex', alignItems:'center', justifyContent:'center', padding:0 }}>‹</button>
        <button onClick={() => setSlide(s => (s + 1) % banners.length)}
          style={{ position:'absolute', right:0, top:'50%', transform:'translateY(-50%)', background:'rgba(255,255,255,.82)', border:'none', width:28, height:52, cursor:'pointer', fontSize:22, color:'#555', display:'flex', alignItems:'center', justifyContent:'center', padding:0 }}>›</button>
        <div style={{ position:'absolute', bottom:8, left:'50%', transform:'translateX(-50%)', display:'flex', gap:5 }}>
          {banners.map((_, i) => (
            <div key={i} onClick={() => setSlide(i)}
              style={{ width: i===slide?18:7, height:7, borderRadius:4, background: i===slide?'#2874f0':'rgba(255,255,255,.75)', transition:'all .3s', cursor:'pointer' }} />
          ))}
        </div>
      </div>

      {/* ── Deals of the Day ── */}
      <div style={{ background:'#fff', padding:'14px 16px 16px', marginTop:6, textAlign:'center' }}>
        <div style={{ fontSize:18, fontWeight:700, color:'#2874f0', marginBottom:6 }}>Deals of the Day</div>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, marginBottom:12 }}>
          <span style={{ fontSize:20 }}>⏰</span>
          <span style={{ fontSize:26, fontWeight:900, color:'#e53935', letterSpacing:1, fontVariantNumeric:'tabular-nums' }}>{countdown}</span>
        </div>
        <div style={{ display:'inline-block', border:'1.5px solid #e53935', color:'#e53935', borderRadius:3, padding:'6px 22px', fontSize:13, fontWeight:700, letterSpacing:1 }}>
          SALE IS LIVE
        </div>
      </div>
            ...
            {filtered.map(p => {
              ...
              return (
                <div key={p._id} onClick={() => navigate(`/product/${p._id}`)}
                  ...
                  <div style={{ position:'absolute', top:8, right:10, color:'#ccc', fontSize:20 }}>♡</div>
                  <div style={{ width:'100%', minHeight:155, display:'flex', alignItems:'center', justifyContent:'center', marginBottom:10 }}>
                    <img src={img} alt={p.name} style={{ maxWidth:'100%', maxHeight:155, objectFit:'contain' }}
                  ...
                  <div style={{ fontSize:13, color:'#212121', lineHeight:1.4, marginBottom:6, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>{p.name}</div>
